...
def pagar_propinas(request, num):
    ...
    if request.method == 'POST':
        ...
        try:
            p = Prestacao.objects.get(usuario=estudante)
            p.prestacao += int(_prestacao)
            presta = p.prestacao
            p.save()

            if presta >= pagamento.prestacao:
                messages.success(request, "Parabens (%s), já completaste o pagamento de propinas." % estudante.perfil)
                return HttpResponseRedirect('/painel/propinas/pagar/%s/' % estudante.id)
            else:
                divida = pagamento.prestacao - presta
            ...
            p = Prestacao.objects.create(usuario=estudante, prestacao=1)
